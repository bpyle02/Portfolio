            ...
            <div className = "flex items-center justify-center">
                ...
                {projects.map((project) => (
                    <article className = "rounded-xl max-w-sm bg-white dark:bg-zinc-950 shadow-xl dark:shadow-gray-100/10">
                        ...
                        <div className = "p-6">
                            <p className = "text-base font-medium mb-1 dark:text-gray-100">{new Date(project.date).toLocaleDateString()}</p>
                            <p className = "text-2xl font-semibold mb-3 dark:text-gray-100">{project.title}</p>
                            {project.tags.map((tags) => (
                                <div className = "inline-block">                                    
                                    <p className = "px-2 inline-flex mb-2 mr-2 rounded-2xl hover:bg-white bg-gray-100 dark:hover:bg-zinc-950 dark:bg-zinc-800 dark:text-white duration-300 transition-colors cursor-pointer">{tags}</p>
                                </div>
                            ))}
